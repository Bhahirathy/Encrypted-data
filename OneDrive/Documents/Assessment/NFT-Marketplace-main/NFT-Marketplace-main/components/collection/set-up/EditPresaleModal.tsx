...
type EditPresaleModalProps = {
    setPresale: React.Dispatch<React.SetStateAction<any[]>>,
    index: number,
    presale: any[]
};

export default function EditPresaleModal({ setPresale, index, presale }: EditPresaleModalProps) {
    ...
    useEffect(() => {
        if (presale && presale[index]) {
            const curData = presale[index];
            setPresaleName(curData.name);
            setSalePrice(curData.price);
            setDuration(curData.duration);
            setWalletMintLimit(curData.walletMintLimit);
        }
    }, [index, presale]);
    ...
    const handleEditPresale = () => {
        const updatedPresaleData = {
            name: presaleName,
            price: salePrice,
            duration: duration,
        ...
        if (index >= 0 && index < existingPresales.length) {
            existingPresales[index] = updatedPresaleData;
            localStorage.setItem('presales', JSON.stringify(existingPresales));
        }
    
        // Update the presale state
        setPresale(prev => {
            const updatedPresales = [...prev];
            if (index >= 0 && index < updatedPresales.length) {
                updatedPresales[index] = updatedPresaleData;
            }
    ...
    return (
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
                <div className="rounded-lg flex items-center" onClick={() => setDialogOpen(true)}>
                    <FontAwesomeIcon
                        icon={faPen}
            ...
            <DialogContent className="w-[436px] rounded-2xl p-0">
                <DialogHeader className="border-b border-grey px-5 py-4">
                    <DialogTitle>Edit a presale</DialogTitle>
                </DialogHeader>
                <div className="px-6 pb-7 pt-2 flex flex-col gap-6">
                    ...
                    <div>
                        ...
                        <div className="mt-3 text-sm rounded-lg text-dark-grey">
                            You can set specific mint limits and prices per wallet, which will override the global sale price and mint limit above for the specified wallets.
                        </div>
